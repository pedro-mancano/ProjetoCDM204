...
function draw(elapsed) {
  ...
  const endX = R + r * Math.cos((-angle + 359) * Math.PI / 180);
  const endY = R + r * Math.sin((-angle + 359) * Math.PI / 180);

  // Create a linear gradient
  const gradient = ctx.createLinearGradient(startX, startY, endX, endY);
  gradient.addColorStop(0, '#54c638'); // Start with red
  gradient.addColorStop(1, '#54c63800'); // End with blue
  ...
  ctx.beginPath();
  ctx.moveTo(R, R);
  ctx.arc(R, R, r, -angle * Math.PI / 180, (-angle + 360) * Math.PI / 180);
  ctx.fill();
  ctx.globalAlpha = 1;
}
