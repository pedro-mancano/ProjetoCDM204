...
function draw(elapsed) {
  ...
  ctx.lineWidth = 1;
  for (let i = 0; i < R; i += R / 10) {
    for (let j = 0; j < R; j += R / 10) {
      ctx.beginPath();
      ctx.moveTo(i, 0);
      ctx.lineTo(i, R);
      ctx.stroke();
  ...
  ctx.beginPath();
  ctx.arc(R / 2, R / 2, 200, 0, 2 * Math.PI);
  ctx.fill();
  ...
  for (let i = 0; i < 360; i += 90) {
    ctx.beginPath();
    ctx.moveTo(R / 2, R / 2);
    ctx.lineTo(R / 2 + 200 * Math.cos(i * Math.PI / 180), R / 2 + 200 * Math.sin(i * Math.PI / 180));
    ctx.stroke();
  }
  ...
  for (let i = 0; i < 6; i++) {
    ctx.beginPath();
    ctx.arc(R / 2, R / 2, R / 10 * i, 0, 2 * Math.PI);
    ctx.stroke();
  }
}
