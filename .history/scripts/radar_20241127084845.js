...
function draw(elapsed) {
  ...
  ctx.beginPath();
  ctx.moveTo(R, R);
  ctx.lineTo(R + r * Math.cos((performance.now() / 1000) % (2 * Math.PI)), R + r * Math.sin((performance.now() / 1000) % (2 * Math.PI)));
  ctx.stroke();

  // Draw the radar sweep "shadow" (fading trail)
  ctx.strokeStyle = '#54c638';
  ctx.lineWidth = 2 * scaleFactor;
  ctx.globalAlpha = 0.5;
  ctx.beginPath();
  ctx.moveTo(R, R);
  ctx.lineTo(R + r * Math.cos((performance.now() / 1000 - Math.PI) % (2 * Math.PI)), R + r * Math.sin((performance.now() / 1000 - Math.PI) % (2 * Math.PI)));
  ctx.stroke();
  ctx.globalAlpha = 1;
}
